...
//				bean.setPART_TYPE(rs.getString("PART_TYPE"));
	...
	/*____________총 일정 수_____________*/
	public int getTotalShcedule() {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			...
			sql = "select count(*) form schedule";
			pstmt=con.prepareStatement(sql);
	...
	/*_____________일정 상세 조회 schedyleDetail.jsp_____________*/
	...
	public void newSchedule(HttpServletRequest req) {
		...
		try {
			...
			sql = "INSERT INTO schedule(SCHE_NAME, SCHE_DATE, SCHE_START_DATE, SCHE_START_TIME,"
					+ " SCHE_END_DATE, SCHE_END_TIME, SCHE_DETAIL, SCHE_FILE, MEM_NO, TYPE_NO, PART_NO)";
			sql+= "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			...
			pstmt.setString(4, multi.getParameter("SCHE_START_TIME"));
			pstmt.setString(5, multi.getParameter("SCHE_END_DATE"));
			pstmt.setString(6, multi.getParameter("SCHE_END_TIME"));
			pstmt.setString(7, multi.getParameter("SCHE_DETAIL"));
			pstmt.setString(8, multi.getParameter("SCHE_FILE"));
	...
	public Vector<ScheduleBean> getEndTimeList(){
		...
		try {
			...
			while(rs.next()) {
				...
				endBean.setST_NO(rs.getInt("END_NO"));
				endBean.setST_TIME(rs.getString("END_TIME"));
				
				System.out.println("ScheduleMgr.java" + rs.getString("END_TIME"));
