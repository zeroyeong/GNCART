...
public class NoticeMgr {
	...
	public Vector<NoticeBean> getBoardList() {
	    ...
	    try {
	        ...
	        sql = "SELECT n.*, m.MEM_NAME, ns.not_sub FROM notice n JOIN member m ON n.MEM_NO = m.MEM_NO JOIN noticesub ns ON n.NOTSUB_NO = ns.NOTSUB_NO ORDER BY n.NOT_NO DESC";
	...
	public void insertNotice(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert notice(NOT_NO,NOT_TITLE,NOT_CONTENT,NOT_DATE,NOT_HIT,NOT_FILENAME,NOT_FILESIZE,MEM_NO,NOTSUB_NO)";
			sql += "values(?,?,?,now(),0,?,?,?,?)";
	...
	public NoticeBean getNotice(int NOT_NO) {
	    ...
	    try {
	        con = pool.getConnection();
	        sql = "SELECT n.*, m.MEM_NAME, ns.NOT_SUB FROM notice n JOIN member m ON n.MEM_NO = m.MEM_NO JOIN noticesub ns ON n.NOTSUB_NO = ns.NOTSUB_NO WHERE n.NOT_NO=?";
	        pstmt = con.prepareStatement(sql);
	        pstmt.setInt(1, NOT_NO);
	        rs = pstmt.executeQuery();
	...
	public void reviseNotice(NoticeBean bean) {
		...
		try {
			...
			sql = "update notice set NOT_TITLE=?,NOT_CONTENT=?,MEM_NO=?,NOTSUB_NO=? where NOT_NO=?";
